...
- 明确 AI 助手下一步先做 P0 稳定闭环，而不是继续堆更多图形展示能力。
- 新增 `docs/ai-assistant-p0.md` 和 `docs/en/ai-assistant-p0.md`。
- 将 AI 助手 P0 页面加入 VitePress 中英文导航。
...
- AI 后端、模型、登录态错误可见。
- 发送、停止、重试、重新生成状态一致。
- iPad 横屏、竖屏、分屏下输入框和消息布局稳定。

下一步：

- 先补 P0 发送状态和流式边界测试，再收紧 `AssistantStore` 的并发发送、停止和失败状态。

## 2026-06-22

完成事项：

- 新增 `docs/ai-assistant-p1.md` 和 `docs/en/ai-assistant-p1.md`，明确 AI 助手 P1 聊天核心体验的范围、验收方式和后续边界。
- 将 P1 页面加入 VitePress 中英文导航，并在路线图中补充 P1 入口。
- iPad 端模型选择改为优先读取后端 `/api/assistant/models`，接口不可用时回退到本地默认模型列表。
- 新增继续生成、对话本地自动标题、会话搜索、附件预览和附件大小展示。
- 每条消息已有复制、重试、重新生成、继续生成入口，附件保留上传中、失败重试和大小限制状态。
- 补充 P1 定向测试，覆盖动态模型、降级模型、继续生成、自动标题、搜索和附件草稿展示。

当前边界：

- 新对话标题目前先在 iPad 本地生成，后续可以接入后端标题生成接口。
- 会话搜索当前覆盖本地已加载会话和消息，后续大量历史数据需要后端分页搜索。
- 附件上传仍以当前后端上传契约为准，后续需要补真实文件持久化、类型白名单和服务端扫描状态。

下一步：

- 跑完整构建与测试后，继续做 AI 助手 P1 剩余闭环：真实取消/停止、附件后端持久化、会话管理服务端一致性。
